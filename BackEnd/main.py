...
@app.post('/import_csv')
def import_csv(file: UploadFile = File(...), db: Session = Depends(get_db), current_user=Depends(get_current_admin)):
    """Importar profesoras desde un CSV simple.
    Formato esperado: nombre,email,especialidad (sin cabecera o con cabecera).
    Devuelve resumen con cantidad creada y errores por línea.
    Requiere rol admin.
    """
    ...
    for row in reader:
        ...
        if not row or all([not c.strip() for c in row]):
            continue
        # try to handle header row
        if line_no == 1 and ('@' not in ''.join(row)):
            # probably header, skip
            continue
        try:
            # expecting at least nombre,email,especialidad
            nombre = row[0].strip()
            email = row[1].strip()
            especialidad = row[2].strip() if len(row) > 2 else 'General'
            # validar existencia
            exists = db.query(Profesora).filter(Profesora.email == email).first()
            if exists:
                errors.append({'line': line_no, 'email': email, 'error': 'Ya existe'})
                continue
            # crear con contraseña aleatoria temporal (debe resetearse)
            from auth import get_password_hash
            temp_pass = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(10))
            hashed = get_password_hash(temp_pass)
            p = Profesora(nombre=nombre, email=email, hashed_password=hashed, especialidad=especialidad)
            db.add(p)
            db.commit()
            created += 1
